...
function checkIfIdentical(A, B) {
  ...
  const normalizedA = normalizeExpression(A);
  const normalizedB = normalizeExpression(B);

  // Compare the normalized expressions
  for (let key in normalizedA) {
    if (normalizedA[key] !== normalizedB[key]) {
      return 0;
    }
  }

  for (let key in normalizedB) {
    if (normalizedB[key] !== normalizedA[key]) {
      return 0;
    }
  }
...
let A = '-(a+b+c)';
let B = '-a-b-c';
console.log(checkIfIdentical(A, B)); // Output: 1

// A = 'a-b-(c-d)';
// B = 'a-b-c-d';
// console.log(checkIfIdentical(A, B)); // Output: 0
