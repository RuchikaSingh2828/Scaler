...
// Starting from the first row to the second last row of the matrix, for each element, say A[i][j] of the current row in the matrix,
// find the smallest element in the next row which is greater than or equal to the current element, say it a (lower bound)
// and the largest element which is smaller than the current element, say it b.
// This can be done using Binary search (the reason we sorted each row of the matrix) or using the in-built function of lower_bound.
...
module.exports = {
  ...
  // Initialize one variable “ans” as a large value to track the latest minimum difference between two elements.
  // Starting from the first row to the second last row of the matrix, for each element, say A[i][j] of the current row in the matrix,
  // find the smallest element in the next row which is greater than or equal to the current element, say it a (lower bound) and the largest element which is smaller than the current element, say it b.
  // This can be done using Binary search (the reason we sorted each row of the matrix) or using the in-built function of lower_bound.
