# Problem Description

# Given an array of integers A of size N and an integer B.

# In a single operation, any one element of the array can be increased by 1. You are allowed to do at most B such operations.

# Find the number with the maximum number of occurrences and return an array C of size 2, where C[0] is the number of occurrences, and C[1] is the number with maximum occurrence.
# If there are several such numbers, your task is to find the minimum one.



# Problem Constraints

# 1 <= N <= 105

# -109 <= A[i] <= 109

# 0 <= B <= 109



# Input Format

# The first argument given is the integer array A.
# The second argument given is the integer B.



# Output Format

# Return an array C of size 2, where C[0] is number of occurence and C[1] is the number with maximum occurence.



# Example Input

# Input 1:

#  A = [3, 1, 2, 2, 1]
#  B = 3
# Input 2:

#  A = [5, 5, 5]
#  B = 3


# Example Output

# Output 1:

#  [4, 2]
# Output 2:

#  [3, 5]


# Example Explanation

# Explanation 1:

#  Apply operations on A[2] and A[4]
#  A = [3, 2, 2, 2, 2]
#  Maximum occurence =  4
#  Minimum value of element with maximum occurence = 2
# Explanation 2:

#  A = [5, 5, 5]
#  Maximum occurence =  3
#  Minimum value of element with maximum occurence = 5

def max_occurrence_after_increments(A, B):
    A_sorted = sorted(A)
    n = len(A_sorted)
    l = 0
    window_sum = 0
    max_freq = 1
    best_num = A_sorted[0]

    for r in range(n):
        window_sum += A_sorted[r]

        while A_sorted[r] * (r - l + 1) - window_sum > B:
            window_sum -= A_sorted[l]
            l += 1

        # recompute cost to ensure it's valid
        cost = A_sorted[r] * (r - l + 1) - window_sum
        if cost <= B:
            freq = r - l + 1
            if freq > max_freq or (freq == max_freq and A_sorted[r] < best_num):
                max_freq = freq
                best_num = A_sorted[r]

    return [max_freq, best_num]



# Example usage:
# A1 = [3, 1, 2, 2, 1]
# B1 = 3
# Output: [4, 2]  

A1 = [-5,6,1,-5,1,-3,1,3,1,5]
B1 = 9
print(max_occurrence_after_increments(A1, B1))  # [5,1]