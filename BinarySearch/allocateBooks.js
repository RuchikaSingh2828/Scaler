...
function allocateBooks(A, B) {
  const n = A.length;
  if (n < B) return -1; // not enough books to allocate

  let left = Math.max(...A); // minimum possible max pages
  let right = A.reduce((sum, pages) => sum + pages, 0); // maximum possible max pages

  let result = right;
  while (left <= right) {
    const mid = Math.floor((left + right) / 2);
    if (canAllocate(A, B, mid)) {
      result = mid; // found a valid allocation
...
function canAllocate(books, students, maxPages) {
  ...
  for (let pages of books) {
    if (currentSum + pages > maxPages) {
      requiredStudents++;
      currentSum = pages; // allocate to next student
...
console.log(allocateBooks([12, 15, 78], 4)); // Output: -1
