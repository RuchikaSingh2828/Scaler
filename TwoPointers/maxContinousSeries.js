...
function maxContinousSeries(A, B) {
  let left = 0;
  let right = 0;
  let zeroCount = 0;
  let maxLength = 0;
  ...
  const result = [];
  for (let i = bestLeft; i < bestLeft + maxLength; i++) {
    if (A[i] === 1 || (A[i] === 0 && B > 0)) {
      result.push(i);
      if (A[i] === 0) {
        B--;
...
console.log(maxContinousSeries([1, 1, 0, 1, 1, 0, 0, 1, 1, 1], 1)); // [0, 1, 2, 3, 4]
console.log(maxContinousSeries([1, 0, 0, 0, 1, 0, 1], 2)); // [3, 4, 5, 6]
