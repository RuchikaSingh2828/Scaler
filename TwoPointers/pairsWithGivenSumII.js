...
function countPairsWithSum(A, B) {
  let left = 0,
    right = A.length - 1,
    count = 0;
  ...
  while (left < right) {
    ...
    if (sum === B) {
      ...
      while (left < right && A[left] === x) countx++, left++;
      while (left <= right && A[right] === y) county++, right--;
...
console.log(countPairsWithSum([1, 2, 6, 6, 7, 9, 9], 13)); // Output: 2
